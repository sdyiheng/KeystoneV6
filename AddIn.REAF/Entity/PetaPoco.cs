...
namespace PetaPoco
{
	...
	public class Database : IDisposable
	{
		...
		public Database(string connectionStringName)
		{
            ...
            //System.Configuration.ConfigurationManager.ConnectionStrings.r
        ...
        string AddSelectClause(Type t, string sql)
        {
            if (sql.StartsWith(";"))
                return sql.Substring(1);
        ...
        public List<object> Fetch(Type t, string sql, params object[] args)
        {
            List<object> list = new List<object>();
            foreach (object obj in Query(t, sql, args))
                list.Add(obj);
            return list;
        }
        public List<object> Fetch(Type t, Sql sql)
        {
            return Fetch(t,sql.SQL, sql.Arguments);
        }
        ...
        public void BuildPageQueries(Type t, long skip, long take, string sql, ref object[] args, out string sqlCount, out string sqlPage)
        {
            // Add auto select clause
            if (EnableAutoSelect)
                sql = AddSelectClause(t, sql);
        ...
        public Page Page(Type t, long page, long itemsPerPage, string sql, params object[] args)
        {
            string sqlCount, sqlPage;
            BuildPageQueries(t, (page - 1) * itemsPerPage, itemsPerPage, sql, ref args, out sqlCount, out sqlPage);
		...
		public Page Page(Type t, long page, long itemsPerPage, Sql sql) 
		{
			return Page(t, page, itemsPerPage, sql.SQL, sql.Arguments);
		}
        ...
        public IEnumerable Query(Type t, string sql, params object[] args)
        {
            if (EnableAutoSelect)
                sql = AddSelectClause(t, sql);
            ...
            try
            {
                using (var cmd = CreateCommand(_sharedConnection, sql, args))
                {
                    ...
                    var factory = pd.GetFactory(cmd.CommandText, _sharedConnection.ConnectionString, ForceDateTimesToUtc, 0, r.FieldCount, r) 
                        as Func<IDataReader, Object>;
                    using (r)
                    {
                        while (true)
                        {
        ...
        public IEnumerable<DynObj> Query(string sql, params object[] args)
        {
            OpenSharedConnection();
                    ...
                    using (r)
                    {
                        while (true)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    poco.SetPropertyValue(r.GetName(i), r.GetValue(i));
                                }
                            }
                            catch (Exception x)
                            {
		...
		public class PocoData
		{
			...
			private static Func<object, object> GetConverter(bool forceDateTimesToUtc, PocoColumn pc, Type srcType, Type dstType)
			{
                ...
                if (srcType == typeof(string) && dstType == typeof(Guid))
                {
                    converter = delegate(object src) { return Guid.Parse(src.ToString()); };
                }

                if (srcType == typeof(Guid) && dstType == typeof(string))
                {
                    converter = delegate(object src) { return src.ToString().ToLower(); };
                }
