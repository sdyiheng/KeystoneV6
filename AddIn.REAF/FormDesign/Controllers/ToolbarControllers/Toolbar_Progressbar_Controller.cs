...
{
    class Toolbar_Progressbar_Controller : mouseActionToolbarActionController
    {
        public Toolbar_Progressbar_Controller(System.Windows.Forms.ToolStripButton _btn)
            : base(_btn)
        {
        }
        ...
        protected override void doMyClickAction(Keystone.AddIn.FormDesigner.Messages.ToolbarCommandClickedMsg msg)
        {
            if (this.button.Checked)
            {
                ...
                if (this.element != null && this.element is ProgressElement && !multiTimes)
                    return;

                SuperMCMService.PostMessage(new InsertElementTypeMsg(new ProgressElement(), msg.ClickType == MouseClickType.DoubleClick), ViewDesignerMainController.MESSAGECHANNEL);
            }
        }
        ...
        private void on(InsertElementTypeMsg msg)
        {
            ...
            this.button.Checked = msg.Element is ProgressElement;
        } 
    }
}
