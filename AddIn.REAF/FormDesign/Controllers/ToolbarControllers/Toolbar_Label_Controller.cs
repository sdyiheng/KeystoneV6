...
{
    class Toolbar_Label_Controller : mouseActionToolbarActionController
    {
        public Toolbar_Label_Controller(System.Windows.Forms.ToolStripButton _btn)
            : base(_btn)
        {
        }
        ...
        protected override void doMyClickAction(Keystone.AddIn.FormDesigner.Messages.ToolbarCommandClickedMsg msg)
        {
            if (this.button.Checked)
            {
                ...
                if (this.element != null && this.element is LabelElement && !multiTimes)
                    return;

                SuperMCMService.PostMessage(new InsertElementTypeMsg(new LabelElement(), msg.ClickType == MouseClickType.DoubleClick), ViewDesignerMainController.MESSAGECHANNEL);
            }
        }
        ...
        private void on(InsertElementTypeMsg msg)
        {
            ...
            this.button.Checked = msg.Element is LabelElement;
        } 
    }
}
