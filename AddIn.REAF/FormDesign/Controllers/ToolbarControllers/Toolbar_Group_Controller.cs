...
{
    class Toolbar_Group_Controller : mouseActionToolbarActionController
    {
        public Toolbar_Group_Controller(System.Windows.Forms.ToolStripButton _btn)
            : base(_btn)
        {
        }
        ...
        protected override void doMyClickAction(Keystone.AddIn.FormDesigner.Messages.ToolbarCommandClickedMsg msg)
        {
            if (this.button.Checked)
            {
                ...
                if (this.element != null && this.element is GroupElement && !multiTimes)
                    return;

                SuperMCMService.PostMessage(new InsertElementTypeMsg(new GroupElement(), msg.ClickType == MouseClickType.DoubleClick), ViewDesignerMainController.MESSAGECHANNEL);
            }
        }
        ...
        private void on(InsertElementTypeMsg msg)
        {
            ...
            this.button.Checked = msg.Element is GroupElement;
        } 
    }
}
